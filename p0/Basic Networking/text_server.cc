 ...
 * @brief Text_server is half of a client/server pair that shows how to receive text
...
/**
 * @brief Print an error message that combines some provided text (prefix) with the standard unix erro
 * message that accompanies errno, and then exit the program. This routine makes it easier to see the logic in our program
 * while still correctly handling errors.
 * 
...
/**
 * @brief Create a server socket that we can use to listen for new incoming requests
 * 
...
int create_server_socket(std::size_t port){
    ...
    // The default is that when the server crashes, the socket can not be used for a
...
/**
 * @brief Receive text over the provided socket file descriptor, and send it back to
...
void echo_server(int sd, bool verbose){
    // vers for tracking connection duration, bytes transmitted
    size_t xmitBytes = 0;
    ...
    while(true){
        ...
        // that it is easier to see how the server handles full buffers
        char buf[16] = {0};
        ...
        if(rcd <= 0){
            if(errno != EINTR){
                if(rcd == 0){
                    ...
                    //     something bad. That is not a good way to write reliable code. 
